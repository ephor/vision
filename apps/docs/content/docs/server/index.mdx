...
Forward observability data to any OpenTelemetry-compatible backend — BetterStack, Honeycomb, Grafana Tempo, Datadog, an OTel Collector — via the `exporters` config. Traces and logs are configured independently under the same `exporters` object. Export runs alongside the local Dashboard.
...
Each HTTP request becomes a synthetic root span (`SERVER`) with `c.span(...)` calls as nested `INTERNAL` spans, and trace logs attached as span events.

### Log Export

All `console.*` calls captured by Vision can be forwarded as OTLP LogRecords. Logs inside a trace carry the `traceId` for backend correlation.
...
Traces and logs work side by side:
...
Both exporters share the same options and behaviour: OTLP/JSON over HTTP, buffered batch flushing, automatic retry on failure, and full error isolation.

For custom sinks, implement `TraceExporter` or `LogExporter` and add them to the respective array.
