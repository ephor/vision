...
Vision follows the **Wide Events** philosophy. Instead of manually adding IDs to every log message, you add context once to the request scope, and it's automatically attached to every log and span in that trace.
...
app.service('orders')
  .endpoint('POST', '/orders', schema, async (data, c) => {
    ...
    // Add context once - available in all logs and spans
    c.addContext({ 
      orderId, 
      userId: data.userId,
      'request.type': 'order_creation'
    })
    
    console.log('Creating order') // automatically has orderId & userId
    
    const order = c.span('db.insert', {}, () => {
      console.log('Inserting into database') // shares the same context
...
**Dashboard shows:**
```text
INF Creating order              [orderId=ord_123] [userId=user_456]
INF Inserting into database     [orderId=ord_123] [userId=user_456]
INF Order created successfully  [orderId=ord_123] [userId=user_456]
```

This context is also indexed for search.
