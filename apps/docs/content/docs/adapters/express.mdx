...
app.post('/users', validator('body', CreateUser), (req, res) => res.status(201).json(req.body))
...
app.put('/users/:id', validator('body', UpdateUser), (req, res) => {
  res.json(req.body)
})
```

The universal `validator()` supports Zod, Valibot, and other Standard Schema-compatible libraries. It attaches the schema to route metadata so API Explorer can generate a commented JSON template. Comments are stripped automatically when sending.
...
- API Explorer shows “No schema defined” → ensure the route uses `validator('body', schema)` and `enableAutoDiscovery(app)` ran after route registration
